...
bool FileUtil::DeleteFile(const char* filename) {
  if (Exists(filename)) {
    ...
    if (result != 0) {
      Util::Log(EVENT_ERROR, "Failed to remove file %s. (%d)", filename, errno);
    }
    return result == 0;
  }
...
bool FileUtil::CopyFile(const char* src, const char* dest) {
  ...
  if (!success) {
    Util::Log(EVENT_ERROR, "Failed copy (%s) to (%s). (%d)", src, dest, errno);
  }
...
bool FileUtil::MoveFile(const char* src, const char* dest) {
  if (!FileUtil::CopyFile(src, dest)) {
    Util::Log(EVENT_ERROR, "Failed move file because of copy failure.");
    return false;
  }
...
bool FileUtil::MakeTemp(std::string* temp) {
  ...
  if (size == 0 || size >= 1024) {
    Util::Log(EVENT_ERROR, "Failed to get win dir. (%d).", GetLastError());
    return false;
  }
  temp_dir.append(win_dir).append("\\Temp\\");

  // Create a temp file name.
  char temp_file[32] = "sitemap_tmp_XXXXXX";
  if (_mktemp(temp_file) == NULL) {
    Util::Log(EVENT_ERROR, "Failed to create temp file. (%d)", errno);
    return false;
  }
...
bool FileUtil::CreateDir(const char* dirname) {
  ...
  if (!result) {
    if (GetLastError() == 183) {
      ...
      Util::Log(EVENT_ERROR, "Failed to create dir [%s]. (%d)",
...
bool FileUtil::ListDir(const char* dir, bool onlyname,
  ...
  HANDLE handle = FindFirstFileA(pattern.c_str(), &finddata);
  if (handle == INVALID_HANDLE_VALUE) {
    Util::Log(EVENT_ERROR, "Failed to list dir %s. (%d).",
              dir, GetLastError());
    return false;
  }
...
bool FileUtil::GetFileAttribute(const char* path, FileAttribute* attr) {
  ...
  if (stat(path, &filestatus) != 0) {
    Util::Log(EVENT_ERROR, "Failed to stat file [%s] (%d).",
              path, errno);
    return false;
  }
