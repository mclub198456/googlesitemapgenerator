...
bool Thread::Start() {
  ...
  if (stop_event_ == NULL) {
    Util::Log(EVENT_ERROR, "Failed to create stop event for thread. (%d).",
              GetLastError());
    return false;
  } else {
  ...
  if (thread_ == NULL) {
    Util::Log(EVENT_ERROR, "Failed to create thread. (%d).", GetLastError());
    return false;
  }
  ...
  if (result != 0) {
    Util::Log(EVENT_ERROR, "Failed to create thread. (%d).", result);
    return false;
  }
#endif
  return true;
...
void Thread::Stop() {
  ...
  if (!SetEvent(stop_event_)) {
    Util::Log(EVENT_ERROR, "Failed to set event to stop thread. (%d)",
              GetLastError());
  }
#elif defined(__linux__)
...
void Thread::WaitOrDie(int seconds) {
  ...
  if (res == WAIT_FAILED) {
    Util::Log(EVENT_ERROR, "Failed to wait thread stop object. (%d)",
              GetLastError());
  ...
  if (sleep(seconds) != 0) {
    Util::Log(EVENT_ERROR, "Failed to wait thread to stop. (%d)", errno);
    thread_ = 0;
...
void Thread::ChangePriority() {
  ...
  if (result == FALSE) {
    Util::Log(EVENT_ERROR, "Failed to change thread priority. (%d)",
              GetLastError());
  }
#elif defined(__linux__)
  int result = 0;
...
void Thread::Join() {
  ...
  if (WaitForSingleObject(thread_, INFINITE) == WAIT_FAILED) {
    Util::Log(EVENT_ERROR, "Failed to wait thread exit, ignore.");
  }
#elif defined(__linux__)
