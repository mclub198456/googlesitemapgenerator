...
bool BaseSitemapService::Initialize(const SitemapSetting* sitemapsetting,
  ...
  // Build url filters.
  includefilter_ = UrlFilterBuilder::Build(sitemapsetting_->included_urls());
  excludefilter_ = UrlFilterBuilder::Build(sitemapsetting_->excluded_urls());

  // Initialize informers.
  const std::vector<Url>& urls = sitemapsetting_->notify_urls();
  ...
  if (!FileUtil::Exists(sitemapfile.c_str())) {
    Util::Log(EVENT_IMPORTANT, "Empty [%s] is generated for initialization.",
              sitemapfile.c_str());
...
void BaseSitemapService::Run() {
  Util::Log(EVENT_CRITICAL, "Start to generate [%s] for site [%s].",
    sitemap_name_.c_str(), sitesetting_.site_id().c_str());

  // Run the service.
  bool result = InternalRun();
  if (result) {
    Util::Log(EVENT_CRITICAL, "Succeeded to generate [%s] for site [%s].",
      sitemap_name_.c_str(), sitesetting_.site_id().c_str());
  } else {
    Util::Log(EVENT_ERROR, "Failed to generate [%s] for site [%s].",
      sitemap_name_.c_str(), sitesetting_.site_id().c_str());
  }
...
bool BaseSitemapService::EndGenerating(bool alwaysindex, bool noindex) {
  if (alwaysindex && noindex) {
    Util::Log(EVENT_ERROR, "always-index and always-no-index conflicted.");
    return false;
  }
  ...
  if ((sitemap_index_.Size() > 1 && noindex == false) || alwaysindex) {
    ...
    if (result == 0) {
      ...
      Util::Log(EVENT_ERROR, "Failed to create sitemap index [%s] for [%s].",
    ...
    if (rename(sitemap0.c_str(), final_file.c_str()) != 0) {
      Util::Log(EVENT_ERROR, "Failed to rename file [%s] to [%s] (%d)\n",
...
bool BaseSitemapService::Flush() {
  ...
  if (sitemap_index_.Size() >= kMaxSitemapIndex) {
    Util::Log(EVENT_ERROR, "Number of indexed sitemap exceeds 1000. Ignore.");
    urlset_.Clear();
    return true;
  }
  ...
  // IO operation failed.
  if (count == -1) {
    Util::Log(EVENT_ERROR, "Failed to write sitemap file [%s].",
      full_path.c_str());
    return false; 
  }

  if (count == 0 && urlset_.Size() != 0) {
    Util::Log(EVENT_ERROR, "Sitemap file size limit is too small.");
    return false;
  }
...
bool BaseSitemapService::CompressFile(const std::string& file) {
  ...
  if (Util::GZip(file.c_str(), compressed.c_str())) {
    ...
    Util::Log(EVENT_ERROR, "Failed to compress file: [%s]. (%d)",
              file.c_str(), errno);
    return false;
  }
}
