...
bool PlainSitemapService::InternalRun() {
  ...
  // Update the database if the database is too old.
  time_t now = time(NULL);
  time_t required_time = now - (sitemapsetting_->update_duration() / 10);
  if (data_manager_->GetLastUpdate() < required_time) {
    if (!data_manager_->UpdateDatabase()) {
      Util::Log(EVENT_ERROR, "Failed to update database.");
      return false;
    }
  }
  ...
  if (!data_manager_->GetHostName(&hostname)) {
    Util::Log(EVENT_ERROR, "Failed to get host name.");
    return false;
  }

  // Lock data base to read.
  if (!data_manager_->LockDiskData(true)) {
    Util::Log(EVENT_ERROR, "Failed to lock site data.");
    return false;
  }
  ...
  do {
    ...
    if (reader->Open(data_base.c_str()) != 0) {
      Util::Log(EVENT_ERROR, "Failed to open data base to read.");
      break;
    }

    // Start to generate sitemap file.
    if (!Start(data_manager_->GetRecordFileStata(), hostname.c_str())) {
      Util::Log(EVENT_ERROR, "Failed to start generate sitemap.");
      break;
    }
    ...
    while (reader->Read(&fprint, &record) == 0) {
      if (!ProcessRecord(record)) {
        Util::Log(EVENT_NORMAL, "Failed to process url [%s] for sitemap, ignore.",
                  record.url);
      }
    }
