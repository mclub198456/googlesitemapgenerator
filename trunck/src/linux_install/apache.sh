...
function AnalyzeApache
{
  ...
  "$APACHE_BIN" -V 1>/dev/null 2>/dev/null
  ...
  cat "$APACHE_CONF" 1>/dev/null 2>/dev/null
  if test $? -eq 0; then :; else
...
function RestartApache
{
  ...
  if test "x$APACHE_VERSION" = "x1.3" ||
    ...
    if [ $APACHE_RUNNING -eq 0 ]; then
      ...
      if kill -HUP $APACHE_PID ; then
        echo "Apache is restarted"
        return 0
      else
        echo "Apache could be restarted"
        return 3
      fi
    fi
  else
