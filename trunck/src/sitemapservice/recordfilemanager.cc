...
bool RecordfileManager::Initialize(const char* dir, const char* siteid,
  ...
  if (!FileUtil::CreateDir(directory_.c_str())) {
    Util::Log(EVENT_ERROR, "Failed initialize record file dir [%s].",
              directory_.c_str());
    return false;
  }
  ...
  if (!FindFiles(directory_, kTempFilePrefix, &file_names)) {
    Util::Log(EVENT_ERROR, "Failed to list old temp files.");
    return false;
  }
  for (int i = 0; i < static_cast<int>(file_names.size()); ++i) {
    ...
    if (!FileUtil::GetFileAttribute(full_name.c_str(), &file_attr)) {
      Util::Log(EVENT_ERROR, "Failed to access file [%s]. Skip.",
        file_names[i].c_str());
    } else {
      TempFile tempfile;
...
bool RecordfileManager::CompleteCurrentFile() {
  ...
  if (rename(currentfile.c_str(), nextfile_full.c_str()) != 0) {
    Util::Log(EVENT_ERROR, "Failed to complete file [%s]. (%d)",
  ...
  if (!FileUtil::GetFileAttribute(nextfile_full.c_str(), &file_attr)) {
    Util::Log(EVENT_ERROR, "Failed to access new temp file [%s].",
              nextfile_full.c_str());
  }
...
void RecordfileManager::CleanUpTempFile() {
  ...
  for (int j = 0; j <= i; ++j) {
    ...
    if (!FileUtil::DeleteFile(full_name.c_str())) {
      Util::Log(EVENT_ERROR, "Failed to delete temp file [%s] (ignore)",
                full_name.c_str());
    }
  }
  ...
  if (i >= 0) {
    ...
    Util::Log(EVENT_IMPORTANT, "[%d] temp files are cleaned from [%s].",
...
bool RecordfileManager::FindFiles(const std::string& dir,
  ...
  if (handle == INVALID_HANDLE_VALUE) {
    if (ERROR_FILE_NOT_FOUND != GetLastError()) {
      Util::Log(EVENT_ERROR, "Failed to list dir with [%s]. (%d)",
...
bool RecordfileManager::FindFiles(const std::string&dir,
  ...
  if (dirhandle == NULL) {
    Util::Log(EVENT_ERROR, "Failed to open dir [%s]. (%d)",
              dir.c_str(), errno);
    return false;
  }
