...
bool FileScanner::InternalRun() {
  ...
  Util::Log(EVENT_CRITICAL, "Start to scan directory (%s) for site (%s).",
            basedir_.c_str(), sitesetting_.site_id().c_str());
  ScanDir(basedir_.c_str(), baseurl_.c_str());
  Util::Log(EVENT_CRITICAL, "End to scan directory (%s).", basedir_.c_str());
...
void FileScanner::ScanDir(const char *dir, const char *url) {
  ...
  if (handle == INVALID_HANDLE_VALUE) {
    Util::Log(EVENT_ERROR, "Failed to scan dir %s. (%d).",
              dir, GetLastError());
    return;
  }
  ...
  do {
    ...
    if (!Util::SystemToUTF8(finddata.cFileName, &utf8_filename)) {
      Util::Log(EVENT_ERROR, "Failed to convert [%s] to utf8.",
                finddata.cFileName);
      continue;
    }
    ...
    if (finddata.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
      ...
      if (FileUtil::GetFileAttribute(childfile.c_str(), &file_attr)) {
        if (file_attr.last_modified > last_run()) {
...
void FileScanner::ScanDir(const char *dir, const char *url) {
  ...
  for (;entry != NULL; entry = readdir(dirhandle)) {
    ...
    if (attr.is_dir) {
      ...
      if (attr.last_modified > last_run()) {
