...
bool BackupService::Initialize(SiteDataManager* datamgr, const SiteSetting& setting,
                               int backup_duration) {
...
void BackupService::Run() {
  ...
  if (last_save + backup_duration_ > now) {
    Util::Log(EVENT_NORMAL, "Data for [%s] is already saved.",
              site_id_.c_str());
    last_run_ = last_save;
    return;
  }
  ...
  if (!sitedata_manager_->SaveMemoryData(false, true)) {
    Util::Log(EVENT_ERROR, "Failed to save memory data for [%s].",
              site_id_.c_str());
  } else {
    Util::Log(EVENT_IMPORTANT, "Save memory data for [%s] successfully.",
              site_id_.c_str());
  }
  time(&last_run_);
}
