...
bool NewsSitemapService::Initialize(SiteDataManager* datamanager,
  ...
  if (!newsdata_manager_->Initialize(datamanager, "news_sitemap")) {
    Util::Log(EVENT_ERROR, "Failed to initialize news data manager [%s].",
...
bool NewsSitemapService::End() {
  ...
  if (FileUtil::Exists(old_news.c_str())) {
    UrlFprint fprint_dummy;
    VisitingRecord record;
    RecordFileReader* reader = RecordFileIOFactory::CreateReader();
    reader->Open(old_news.c_str());
    while (reader->Read(&fprint_dummy, &record) == 0) {
    ...
    Util::Log(EVENT_IMPORTANT, "No old news file exist [%s].", old_news.c_str());
  }
  ...
  if (writer->Open(old_news.c_str()) != 0) {
    Util::Log(EVENT_ERROR, "Failed to save news record to [%s].", old_news.c_str());
  }
  int maxsize = std::min<int>(news_sitemap_setting_.max_file_url_number(),
                              static_cast<int>(records.size()));
  for (int i = 0; i < maxsize; ++i) {
    writer->Write(0, records[i]);
...
bool NewsSitemapService::InternalRun() {
  ...
  if (!data_manager_->GetHostName(&hostname)) {
    Util::Log(EVENT_ERROR, "Failed to get host name for news sitemap.");
    return false;
  }
  ...
  do {
    if (!newsdata_manager_->UpdateData()) {
      Util::Log(EVENT_ERROR, "Failed to get new record for news sitemap.");
      break;
    }

    if (!Start(data_manager_->GetRecordFileStata(), hostname.c_str())) {
      Util::Log(EVENT_ERROR, "Failed to start generate news sitemap.");
      break;
    }

    if (reader->Open(newsdata_manager_->GetDataFile().c_str()) != 0) {
      Util::Log(EVENT_CRITICAL, "No new record in news URL database.");
    } else {
      VisitingRecord record;
      UrlFprint fprint;
