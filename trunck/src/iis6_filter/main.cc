...
BOOL WINAPI DllMain(HINSTANCE handle, DWORD reason, LPVOID) {
  ...
  if (!Util::GetModuleDir(handle, &module_dir)) {
    Util::Log(EVENT_ERROR, "Failed to get sitemap filter dll path.");
    return FALSE;
  }
  module_dir.append("\\google-sitemap-generator.log");
  Util::SetLogFileName(module_dir);

  if (reason == DLL_PROCESS_ATTACH) {
    Util::Log(EVENT_CRITICAL, "Attaching sitemapfilter.dll.");

    sitemap_filter = new SitemapFilter();
    if (sitemap_filter == NULL) {
      Util::Log(EVENT_ERROR, "Failed to create sitemap filter.");
      return FALSE;
    }
    ...
    if (!sitemap_filter->Initialize(handle)) {
      ...
      sitemap_filter = NULL;
      Util::Log(EVENT_ERROR, "Failed to initialize sitemap filter.");
      return FALSE;
    }
  } else if (reason == DLL_PROCESS_DETACH) {
    Util::Log(EVENT_CRITICAL, "Detaching sitemapfilter.dll.");
...
BOOL WINAPI GetFilterVersion(HTTP_FILTER_VERSION* filter_version) { 
  try {
    Util::Log(EVENT_CRITICAL, "Getting filter version information.");

    if (!sitemap_filter->GetFilterVersion(filter_version)) {
      Util::Log(EVENT_ERROR, "GetFilterVersion failed.");      
      return FALSE;
    }
    ...
    Util::Log(EVENT_ERROR, "Exception occured in GetFilterVersion().");
    return FALSE;
  }
} 
...
BOOL WINAPI TerminateFilter(DWORD flags) {
  Util::Log(EVENT_CRITICAL, "Terminating sitemap filter");
	return TRUE;
}
