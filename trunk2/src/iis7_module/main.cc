...
HRESULT __stdcall RegisterModule(DWORD dwServerVersion,
  ...
  if (!module_factory->Initialize()) {
    ...
    Util::Log(EVENT_ERROR, "Failed to intialize SitemapModuleFactory. Skip.");
    return S_OK;
  } else {
...
BOOL WINAPI DllMain(HINSTANCE handle, DWORD reason, LPVOID) {
  ...
  Util::SetLogFileName(module_path);

  // for tracing purpose
  if (reason == DLL_PROCESS_ATTACH) {
    Util::Log(EVENT_CRITICAL, "Attaching SitemapModule.dll.");
  } else if (reason == DLL_PROCESS_DETACH) {
    Util::Log(EVENT_CRITICAL, "Detaching SitemapModule.dll.");
  }
