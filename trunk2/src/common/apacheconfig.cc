...
std::string ApacheConfig::GetLocalHost() {
  ...
  if (gethostname(buffer, sizeof(buffer) - 1) != 0) {
    Util::Log(EVENT_ERROR, "Failed to get local host name.");
  ...
  if (host_entry == NULL) {
    Util::Log(EVENT_ERROR, "Failed to get local host address.");
    return std::string("127.0.0.1");
  }

  // Get name of local host.
  if (strchr(host_entry->h_name, '.') != NULL) {
    return std::string(host_entry->h_name);
  }
  for (char* p = host_entry->h_aliases[0]; p != NULL; ++p) {
    if (strchr(p, '.') != NULL &&
  ...
  Util::Log(EVENT_ERROR, "127.0.0.1 is used as default servername at last.");
  return std::string("127.0.0.1");
}
...
bool ApacheConfig::Load() {
  if (conf_file_path_.length() == 0) {
    Util::Log(EVENT_ERROR, "Apache config file path shouldn't be empty.");
    return false;
  }
...
bool ApacheConfig::Load(const char* configfile) {
  ...
  if (!ProcessFile(configfile)) {
    Util::Log(EVENT_ERROR, "Can't load configuration properly");
    return false;
  }
...
bool ApacheConfig::ProcessFile(const char* filename) {
  ...
  if (fout.fail()) {
    Util::Log(EVENT_ERROR, "Failed to open apache conf file [%s]", filename);
    return false;
  }
  ...
  for (int i = 0, isize = static_cast<int>(addrstrs.size());
    ...
    if (pos == std::string::npos) {
      ...
      if (port_str == "*") {
        ...
        Util::Log(EVENT_ERROR, "Invalid port: [%s].", port_str.c_str());
        return false;
      }
    }
  ...
  if (conf->servername.length() == 0) {
    ...
    if (conf->servername.length() == 0) {
      ...
      if (host_entry == NULL || host_entry->h_addrtype != AF_INET
        ...
        Util::Log(EVENT_ERROR, "Can't resolve host name [%s].", host.c_str());
        return false;
      }
