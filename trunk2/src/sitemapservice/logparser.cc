...
bool LogParser::Initialize(SiteDataManager* processor, const SiteSetting& setting) {
  ...
  if (path_.length() == 0) {
    Util::Log(EVENT_ERROR, "Log path for [%s] site is null.",
 ...
 if (UrlProviderService::RefreshTimeStamp() == false) {
   UpdateLastRun(time(NULL) - 3600 * 24 * 365);
 }
...
bool LogParser::InternalRun() {
  Util::Log(EVENT_CRITICAL, "Start to parse log (%s) for site (%s).",
            path_.c_str(), sitesetting_.site_id().c_str());
  FileAttribute fileattr;
  if (!FileUtil::GetFileAttribute(path_.c_str(), &fileattr)) {
    Util::Log(EVENT_ERROR, "Can't access [%s], and stop log.", path_.c_str());
    return false;
  }
  ...
  if (fileattr.is_dir) {
    if (!FileUtil::ListDir(path_.c_str(), false, NULL, &files)) {
      Util::Log(EVENT_ERROR, "Can't list dir [%s]. and stop scan log.",
                path_.c_str());
      return false;
    }
  } else {
  ...
  if (bestparser_ != NULL) {
    bestparser_->set_oldest(last_run());
  }

  // Iterate all log files.
  for (int i = 0; i < static_cast<int>(files.size()); ++i) {
    if (!FileUtil::GetFileAttribute(files[i].c_str(), &fileattr)) {
      Util::Log(EVENT_ERROR, "Ignore log file: [%s].", files[i].c_str());
      continue;
    }

    // Very old log file.
    if (fileattr.last_modified <= last_run()) {
      continue;
    }
  ...
  Util::Log(EVENT_CRITICAL, "End to parse log (%s).", path_.c_str());
  return true;
}
...
bool LogParser::ParseLogFile(const char* path) {
  ...
  if (file == NULL) {
    Util::Log(EVENT_ERROR, "Failed to open log file [%s]. (%d)",
              path, errno);
    return false;
  }
    ...
    if (bestparser_ == NULL) {
      for (int i = 0; i < static_cast<int>(lineparsers_.size()); ++i) {
        if (lineparsers_[i]->Parse(strbuf.c_str(), &record) !=
          ...
          bestparser_->set_oldest(last_run());
          Util::Log(EVENT_CRITICAL, "[%s] is selected for parser [%s].",
